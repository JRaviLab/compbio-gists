...
###########################
# Created by: Sarah Mentzer, JRaviLab (https://jravilab.github.io) | Summer 2025
# In collaboration with: Abhirupa Ghosh, Janani Ravi (JRaviLab), with inputs from Kevin Fotso, CU Alpine
# This script runs AlphaFold3 PPI predictions
...
# - Logs how long it took and checks for success/failure based on 
...
# Create a log to capture the success of each job
TIMESTAMP=$(date '+%Y-%m-%d_%H-%M-%S')
MAIN_LOG="$PWD/scripts/logs/run_main-remaining.log"

echo "Main log file: $MAIN_LOG"

### SET UP ENVIRONMENT FOR GIVEN JOB ###
...
# Create a variable for the given JSON input
AF3_INPUT_FILE=$(echo "$LINE" | awk -F'\t' '{print $4}')

echo "[$(date)] START Job ${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID} for $PPI_ID (input: $AF3_INPUT_FILE)" >> "$_LOG"
...
JOB_END=$(date +%s)
# Calculate the elapsed time of the job by subtracting start from end
...
# Find the summary confidence file ignoring the case of ppi id
...
if [ -n "$PPI_OUTPUT_DIR" ]; then
    ...
    if [ -n "$OUTPUT_FILE" ]; then
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] SUCCESS for $PPI_ID (Job ${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}) — Elapsed: $JOB_ELAPSED_FMT" >> "$_LOG"
    else
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] FAILURE for $PPI_ID — File missing inside matching dir: ${PPI_ID}_summary_confidences.json" >> "$_LOG"
    fi
else
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] FAILURE for $PPI_ID — Output subdirectory not found under $AF3_OUTPUT_DIR" >> "$MAIN_LOG"
fi
